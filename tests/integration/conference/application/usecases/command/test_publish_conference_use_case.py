...
@pytest.fixture
def publish_conference_dependencies(
    ...
    use_case = PublishConferenceUseCase(conference_repository, uow)
...
@pytest.mark.asyncio
async def test_publish_conference_success(publish_conference_dependencies):
    maker, use_case, read_repository = publish_conference_dependencies
    organizer_id = uuid4()
    conference = create_conference(organizer_id)
    await add_conference(maker, conference)

    command = PublishConferenceCommand(conference_id=conference.conference_id)
...
@pytest.mark.asyncio
async def test_publish_conference_not_found(publish_conference_dependencies):
    _, use_case, _ = publish_conference_dependencies
    nonexistent_id = uuid4()
    command = PublishConferenceCommand(conference_id=nonexistent_id)
...
@pytest.mark.asyncio
async def test_publish_conference_not_draft(publish_conference_dependencies):
    maker, use_case, _ = publish_conference_dependencies
    organizer_id = uuid4()
    conference = create_conference(organizer_id)
    conference.publish()
    await add_conference(maker, conference)
